...
export default function HelpPage() {
  ...
  const restaurantGuide = [
    [lang === "sw" ? "1. Pokea groceries kama ingredients" : "1. Receive groceries as ingredients", lang === "sw" ? "Pokea kuku mzima, mafuta, mchele, viungo, mkaa na mboga kupitia Pokea Bidhaa. Ukilipa taslimu na shift ya Daily Close iko wazi, malipo hayo hupunguza pesa inayotarajiwa kwenye shift. Usiziweke tena kwenye Matumizi ya kawaida, ili gharama isihesabiwe mara mbili." : "Receive whole chicken, oil, rice, spices, charcoal, and vegetables through Receive Stock. If you pay cash while a Daily Close shift is open, the purchase reduces expected drawer cash. Do not also add it as a normal Expense, so the cost is not counted twice."],
    ...
    <PublicPageShell>
      <div className="space-y-8">
        ...
        <section className="border-b py-6">
          <h2 className="text-xl font-semibold">{lang === "sw" ? "Matawi kwenye Pro" : "Branches on Pro"}</h2>
          <p className="mt-2 text-sm">{lang === "sw" ? "Mmiliki anafungua Matawi na kuongeza jina na eneo. Pro inajumuisha maeneo 4 pamoja na duka kuu. Tawi lina bidhaa, mauzo, wateja, madeni, nukuu, wafanyakazi na Daily Close yake. Fungua tawi kwanza kabla ya kuongeza bidhaa au mfanyakazi. Mfanyakazi anaingia kwenye tawi alilopangiwa tu, kulingana na ruhusa zake." : "The owner opens Branches and adds a name and location. Pro includes 4 locations including the main shop. Each location has its own stock, sales, customers, debts, quotations, staff, and Daily Close. Open a branch before adding products or staff. Staff log in to their assigned location only, with their existing permissions."}</p>
          <p className="mt-2 text-sm">{lang === "sw" ? "Hamisha bidhaa kati ya matawi kwa kuchagua bidhaa ya kutoka na ya kupokea yenye kipimo na tarehe ya mwisho sawa. Uhamisho si mauzo wala matumizi. Maliza kusawazisha mauzo ya nje ya mtandao kabla ya kubadilisha tawi. Funga Daily Close kabla ya kuhifadhi tawi; rekodi zake hazifutwi." : "Transfer stock by selecting source and destination products with the same unit and expiry date. Transfers are not sales or expenses. Sync offline sales before switching locations. Close Daily Close sessions before archiving a branch; its records are retained."}</p>
          <p className="mt-2 text-sm">{lang === "sw" ? "Usajili na tarehe ya mwisho ni moja kwa biashara nzima. Eneo la ziada ni TZS 10,000 kwa mwezi. Kwenye Malipo, chagua kuongeza matawi hadi usajili uishe au kuongeza mwezi mwingine. Matawi ya ziada hufunguliwa baada ya malipo kuthibitishwa. Hifadhi matawi yaliyozidi kabla ya kupunguza mpango." : "The whole business shares one subscription and renewal date. Extra locations cost TZS 10,000/month. In Billing, choose extra capacity until current expiry or a monthly renewal. Additional capacity is granted only after verified payment. Archive excess branches before reducing the plan."}</p>
        </section>
